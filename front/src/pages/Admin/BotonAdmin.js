import "../../assets/css/cssAdmin/BotonAdmin.css";
...
function BotonAdmin() {
  ...
  const fetchInforme = async () => {
    ...
    if (informe.message === "OK") {
      const element = document.createElement("a");
      const file = new Blob([JSON.stringify(informe)], { type: "text/plain" });
      ...
      await sleep(4000);
      if (window.descargaCancelada === false) {
        ...
        document.getElementById("cuerpoAdmin").style.display = "flex";
  ...
  return (
    <div>
      <button className="BotonAdmin" onClick={fetchInforme}>
        <svg
          width="15"
          height="15"
          viewBox="0 0 15 15"

...
import "../../assets/css/cssHome/Login.css";
...
function Login() {
  ...
  const enviarDatosLogin = async () => {
    if(datos.correoLogin === "admin") {
      if(datos.password !== "admin"){ 
        alert("La contraseña introducida es incorrecta") 
      }
      else{
        window.usuarioLogueado = datos.correoLogin 
        manager.push("/admin"); 
      }
    }
    else{
